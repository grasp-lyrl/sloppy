...
def sec(model, model_init, rho1, rho2, eigspace_list, eigval_list, num_samples, device, b1 = 10, c1 = 0.05, b2 = 10, c2 = 0.05, delta = 0.025):

    a, epsilon = torch.exp(2*rho1), torch.exp(2*rho2)
    pi = torch.tensor(np.pi)
    kl_1, kl_2 = 0, 0

    for m0, m, xi, es, ev_tuple in zip(
        ...
        q = torch.exp(2*xi)
        p = a*ev + epsilon
        kl_1 += proj_norm(m-m0, es, 1/p)
        kl_2 += torch.sum(q / p) + torch.sum(torch.log(p / q))
        kl_2 += -m.numel()

    kl = (kl_1 + kl_2) / 2
    penalty = 4*torch.log(2*torch.abs(b1*torch.log(c1*a))+2*torch.abs(b2*torch.log(c2/epsilon))) + torch.log(pi**2*num_samples / 6*delta)
...
def train(model,model_init, num_samples, device, train_loader, criterion, optimizer, rho1, rho2, es_list, ev_list, num_classes):

    model.train()
    for (data, targets) in train_loader:

        loss2, kl, kl_1, kl_2, penalty = sec(model, model_init, rho1, rho2, es_list, ev_list ,num_samples, device)
...
def initial(model, model_trained):
    ...
    for p in model.mu_std[1].parameters():
        p.data = 0.5*torch.log(torch.ones(p.shape) / 100).to(device)





def initial1(model, model_trained, ev_list, rho1, rho2):
    ...
    for p, ev_tuple in zip(model.mu_std[1].parameters(), ev_list):
        ev = torch.outer(ev_tuple[0], ev_tuple[1]) if len(ev_tuple) == 2 else ev_tuple[0]
        p.data = 0.5*torch.log((a*ev + epsilon) / 10).to(device)
...
def main():
    ...
    c = Network
    rho1, rho2 = torch.tensor(-0.5).to(device).float(), torch.tensor(-3).to(device).float()
    ...
    num_params = sum(p.numel() for p in model.mu_std[0].parameters())
    print(num_params)

    initial1(model, model_trained, evp_list, rho1, rho2)



    # model_state, rho1, rho2 = torch.load(path + "model_bayes_kfac.pt", map_location='cpu')
    # model.load_state_dict(model_state)
    # model = model.to(device)
    # rho1, rho2 = torch.tensor(rho1).to(device), torch.tensor(rho2).to(device)




    epochs = 250
    rho1.requires_grad, rho2.requires_grad = True, True
    param = list(model.parameters()) + [rho1, rho2]
    ...
    loss2, kl, kl_1, kl_2, penalty = sec(model, model_init, rho1, rho2, esp_list, evp_list, num_true, device)
    print("loss2, kl, kl1, kl2, p", loss2.item(), kl.item(), kl_1.item(), kl_2.item(), penalty.item())
    print("rho1, rho2", rho1.item(), rho2.item())
    ...
    for epoch in range(epochs):
        ...
        train(model,model_init, num_true, device, train_loader, criterion, optimizer, rho1, rho2, esp_list, evp_list, num_classes)
        time_end = time.time()
        ...
        if epoch%20 == 0:
            ...
            print('bd1, bd2, rho1, rho2', bd1.item(), bd2.item(), rho1.item(), rho2.item())
            print("loss2, kl, kl1, kl2, p", loss2.item(), kl.item(), kl_1.item(), kl_2.item(), penalty.item())
            for g in optimizer.param_groups:
                    print(g['lr'])
            ...
            if epoch !=0:
                torch.save((model.state_dict(), rho1.item(), rho2.item()), path + "model_bayes_kfac.pt")
    ...
    stat1 = dict({"dt": dt, "bt":bt, "dv":dv, "bv":bv, "bd":bd ,"loss2":loss2.item(), "kl":kl.item(), "kl_1":kl_1.item(), "kl_2":kl_2.item(), "rho1":rho1.item(), "rho2":rho2.item()})


    print(stat1)


    ev_list = ev_trans(evp_list)
    ev_vec = list_to_vec(ev_list).detach().cpu()
    idx = list(np.flip(ev_vec.numpy().argsort()))
    ev_vec = ev_vec[idx].detach()   # eigen value predicted by kfac in descent order
    ...
    stat2 = dict({"ev_vec":ev_vec, "inv_vec" : inv_vec, "var_vec": var_vec})
    print(stat2)

    torch.save((stat1, stat2), path + "stat_bayes_kfac.pt")
