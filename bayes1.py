...
def main():
    ...
    # model_state, rho = torch.load(path + "model_bayes1.pt", map_location='cpu')
    ...
    hess_diag_list = diag_hess(model.mu_std[0], train_loader_approx, device)
    hess_diag = list_to_vec(hess_diag_list).detach().cpu()
    idx = list(np.flip(hess_diag.numpy().argsort()))
    hess_diag = hess_diag[idx].detach()   # hessian diagonal in descent order


    xi_vec = list_to_vec(model.mu_std[1].parameters()).detach().cpu()
    xi_vec = xi_vec[idx]
    var_vec = torch.exp(2*xi_vec)
    inv_vec = 1 / var_vec   # inverse of diagonal variance in the same order as eigen values

    stat2 = dict({"diag_hess":hess_diag, "inv_vec" : inv_vec, "var_vec": var_vec})
    print(stat2)

    torch.save((stat1, stat2), path + "stat_bayes1.pt")
